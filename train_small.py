...
from vit_model_small import vit_base_patch15_75_120 as vit_model
...
for epoch in range(epochs):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        output = output.view(batch_size, 75*120)
        ...
        loss += loss.item()
    train_loss = loss / len(train_loader) * 100
    if (epoch + 1) % 10 == 0:
        train_losses.append(train_loss)
    scheduler.step()
    print(
        f'Processing: [{epoch + 1} / {vitargs.epochs}] | Loss: {round((train_loss / len(train_loader)).item(), 6)} | Learning Rate: {round(optimizer.state_dict()["param_groups"][0]["lr"],6)} ')
    if (epoch + 1) % 10 == 0:
        model.eval()
...
# 绘制损失函数图像
plt.plot(range(1, vitargs.epochs + 1,10), train_losses, label='Train Loss')
